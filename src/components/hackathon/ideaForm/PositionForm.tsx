      ...
      <div className={styles.positionFormContainer}>
        <FormField label="필요 인원 (본인 포함)" required>
          <FormDropdown
            ...
            // 직군에 따라 최대 팀원 수 다름
            options={Array.from({ length: getMaxCapacity(position.key) + 1 }, (_, i) => ({
              id: i,
              name: i.toString(),
            }))}
        ...
        <FormField label="필요 스택 (최대 5개)">
          <SearchDropdown
            multiple
            items={STACKS_WITH_NAMES.map((stack) => ({ id: stack.id, label: stack.name }))}
            selectedIds={currentValue.required_tech_stacks || []}
            maxSelected={5}
            onChange={(item) => handleChange({ required_tech_stacks: item })}
            disabled={isDisabled}
            inPlaceholder="스택명을 입력해 주세요"
            outPlaceholder="스택을 선택해주세요"
          />
        </FormField>
      </div>
    </div>
